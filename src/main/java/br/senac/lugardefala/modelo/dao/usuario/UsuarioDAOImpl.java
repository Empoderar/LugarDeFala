...
public class UsuarioDAOImpl implements UsuarioDAO {
	...
	public List<Usuario> recuperarUsuariosPorNomes(String nome) {
		...
		try {
			...
			CriteriaQuery<Usuario> criteria = construtor.createQuery(Usuario.class);
			Root<Usuario> raizUsuario = criteria.from(Usuario.class);
			ParameterExpression<String> parametroNome = construtor.parameter(String.class);

			criteria.select(raizUsuario).where(construtor.equal(raizUsuario.get(Usuario_.NOME), parametroNome));
			usuariosPelosNomes = session.createQuery(criteria).setParameter(parametroNome, nome).getResultList();
			session.getTransaction().commit();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {

...
public class Principal {
	...
	public static void main(String[] args) {
		...
		comunidadeDAO.inserir(comunidade2);
		...
		comunidadeDAO.inserir(comunidade3);

		Comunidade comunidade4 = new Comunidade(4, "Violência Patrimonial",
				"Comunidade de relatos sobre a retenção, subtração, ou destruição dos bens pessoais da mulher, feitas Por agressor.");
		comunidadeDAO.inserir(comunidade4);
		...
		comunidadeDAO.inserir(comunidade5);
		...
		contatoDAO.inserir(contatoModeradorAmanda);
		...
		moderadorDAO.inserir(moderadorAmanda);
		...
		contatoDAO.inserir(contatoModeradorBrenda);
		...
		moderadorDAO.inserir(moderadorBrenda);
		...
		contatoDAO.inserir(contatoModeradorLuciene);
		...
		moderadorDAO.inserir(moderadorLuciene);
		...
		contatoDAO.inserir(contatoModeradorMariana);
		...
		moderadorDAO.inserir(moderadorMariana);
		...
		contatoDAO.inserir(contatoModeradorGiovanna);
		...
		moderadorDAO.inserir(moderadorGiovanna);
		...
		contatoDAO.inserir(contatoModeradorMarcella);
		...
		moderadorDAO.inserir(moderadorMarcella);
		...
		denunciaConselhoDAO.inserir(denunciaConselho);
		...
		conselhoDAO.atualizar(conselho1);
		conselhoDAO.atualizar(conselho2);
		conselhoDAO.atualizar(conselho3);
		conselhoDAO.atualizar(conselho4);
		conselhoDAO.atualizar(conselho5);
		conselhoDAO.atualizar(conselho6);

		// Criação de consultas

		// Teste Usuario

		Usuario usuarioPorNome = usuarioDAO.recuperarPorNome("Alice");
		if (usuarioPorNome != null) {
			System.out.println("Buscar usuário Por nome: " + usuarioPorNome.getNome());
		} else {
			System.out.println("Não ha usuario com este nome");
		}

		List<Usuario> usuarios = usuarioDAO.recuperarPorComunidade(comunidade3);
		...
		List<Usuario> usuariosPorConselho = usuarioDAO.recuperarPorConselho(conselho1);
		...
		List<Usuario> usuarios3 = usuarioDAO.recuperarPorRelato(relato2);
		...
		Usuario recuperarUsuarioId = usuarioDAO.recuperarPorId(1L);
		...
		Moderador moderadorPorNome = moderadorDAO.recuperarPorNome("Bruna");
		if (moderadorPorNome != null) {
			System.out.println("Buscar Moderador por nome: " + moderadorPorNome.getNome());
		} else {
			System.out.println("Não ha Moderador com esse nome");
		}

		List<Moderador> moderador = moderadorDAO.recuperarPorId(12L);
...
//		List<Moderador> parametroComunidade = moderadorDAO.recuperarModeradoresPorComunidade(comunidade1);
...
//		}
		...
		List<Conselho> recuperarConselhoPorUsuario = conselhoDAO.recuperarPorUsuario(usuario5);

		for (Conselho conselho : recuperarConselhoPorUsuario) {
			if (conselho != null) {
				System.out.println("Buscar conselho Por usuario: " + conselho.getConteudo());
			} else {
				System.out.println("Esse usuario não posui conselhos");
			}
		}

		List<Conselho> conselhos = conselhoDAO.recuperarPorRelato(relato1);

		for (Conselho conselho : conselhos) {
			if (conselho != null) {
				System.out.println("Buscar conselho Por Relato: " + conselho.getConteudo());
			} else {
				System.out.println("Este relato não possui conselhos");
			}
		}

		Conselho recuperarPorId = conselhoDAO.recuperarPorId(1L);
		if (recuperarPorId != null) {
			System.out.println("Buscar conselho Por id: " + recuperarPorId.getConteudo());
		} else {
		...
		List<Relato> recuperarRelatosPorUsuario = relatoDAO.recuperarPorUsuario(usuario2);

		for (Relato relato : recuperarRelatosPorUsuario) {
			if (relato != null) {
		...
		List<Relato> recuperarRelatosPorCategoria = relatoDAO.recuperarPorCategoria(categoria);
		...
		List<Relato> comunidade = relatoDAO.recuperarPorComunidade(comunidade1);
		...
		Contato contatoRecuperado = contatoDAO.recuperarPorId(1L);
		if (contatoRecuperado != null) {
			System.out.println("Contato usuario Por ID: " + contatoRecuperado.getTelefone());
		} else {
		...
		if (recuperarCategoriaId != null) {
			System.out.println("Recuperar categoria Por id: " + recuperarCategoriaId.getNome());
		} else {
			System.out.println("Sem categoria");
		}

		Categoria recuperarCategoriaRelato = categoriaDAO.recuperarPorRelato(relato4);
		if (recuperarCategoriaRelato != null) {
			System.out.println("Recuperar categoria Por relato: " + recuperarCategoriaRelato.getNome());
		} else {
			System.out.println("Relato não existe");
		}

		Categoria recuperarCategoriaNome = categoriaDAO.recuperarPorNome("Sororidade");
		if (recuperarCategoriaNome != null) {
			System.out.println("Recuperar categoria Por nome: " + recuperarCategoriaNome.getNome());
		} else {
			System.out.println("Sem categoria");
		}

		// Teste Denuncia
		List<DenunciaUsuario> DenunciasUsuarios = denunciaUsuarioDAO.recuperarPorUsuario(usuario5);
		...
		List<DenunciaUsuario> DenunciasUsuarios2 = denunciaUsuarioDAO.recuperarPorStatus(Status.PENDENTE);
		...
		List<DenunciaConselho> denunciasConselhos = denunciaConselhoDAO.recuperarPorRelato(relato1);
		for (DenunciaConselho denunciaConselho1 : denunciasConselhos) {
			System.out.println("Denuncia de conselho Por relato: " + denunciaConselho1.getMotivo());
		}

		List<DenunciaConselho> denunciasConselhos3 = denunciaConselhoDAO.recuperarPorStatus(Status.PENDENTE);
		for (DenunciaConselho denunciaConselho3 : denunciasConselhos3) {
			System.out.println("Denuncia de conselho Por status: " + denunciaConselho3.getMotivo());

		List<DenunciaRelato> denunciaRelatoPorStatus = denunciaRelatoDAO.recuperarPorStatus(Status.PENDENTE);
		for (DenunciaRelato denunciaRelato1 : denunciaRelatoPorStatus) {
			System.out.println("Buscar denuncia de relato Por status: " + denunciaRelato1.getMotivo());
		}

		List<DenunciaRelato> denunciaRelatoPorRelato = denunciaRelatoDAO.recuperarPorRelato(relato1);
		for (DenunciaRelato denunciaRelato2 : denunciaRelatoPorRelato) {
			System.out.println("Buscar denuncia de relato Por relato: " + denunciaRelato2.getMotivo());

		DenunciaModerador denunciaModerador1 = denunciaModeradorDAO.recuperarPorId(2L);
		...
		for (Comunidade comunidades : recuperarComunidadeModerador) {
			if (comunidades != null) {
				System.out.println("Recuperar comunidades Por id do moderador : " + comunidades.getNome());
			} else {
